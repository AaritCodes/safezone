...
export const BACKEND_API_KEY_META_NAME = 'safezone-backend-api-key';
export async function fetchBackendSafetyAssessment(payload = {}, timeoutMs = 5200) {
...
export let googleMapsLoaded = false;
export let googleMapsLoadingPromise = null;
export async function ensureGoogleMapsLoaded() {
  ...
  if (state.googleMapsLoadingPromise) return state.googleMapsLoadingPromise;
  const key = getGoogleApiKey();
  state.googleMapsLoadingPromise = new Promise(resolve => {
  ...
  return state.googleMapsLoadingPromise;
}
export function getGoogleErrorCode(details = {}) {
...
export function getGoogleErrorMessage(code, context, details = {}) {
...
export function createGoogleApiError(context, details = {}) {
...
export async function parseGoogleJsonResponse(response) {
  try {
...
export async function fetchGoogleJson(url, options = {}, timeoutMs = 9000, context = 'request') {
  let response;
  try {
  ...
  if (!response.ok) {
    throw createGoogleApiError(context, {
      ...
      googleStatus: data && data.status ? data.status : data && data.error && data.error.status ? data.error.status : '',
      googleMessage: data && data.error_message ? data.error_message : data && data.error && data.error.message ? data.error.message : ''
    });
  }
...
export function notifyGoogleFallback(error, fallbackProvider) {
...
export function getCurrentRegionHint() {
  if (typeof state.currentCountryCode === 'string' && state.currentCountryCode.trim()) {
    return state.currentCountryCode.trim().toLowerCase();
  }
  return '';
}
...
export async function geocodeQueryWithGoogle(query) {
  ...
  return new Promise(resolve => {
    ...
    geocoder.geocode(request, (results, status) => {
      ...
      if (countryComponent && countryComponent.short_name) {
        state.currentCountryCode = String(countryComponent.short_name).toUpperCase();
      }
      resolve({
        lat: location.lat(),
        lng: location.lng(),
...
export async function reverseGeocodeWithGoogle(lat, lng) {
  ...
  return new Promise(resolve => {
    ...
    geocoder.geocode({
      ...
      if (countryShort) {
        state.currentCountryCode = countryShort;
      }
...
export async function fetchRouteDirectionsWithGoogle(fromLat, fromLng, toLat, toLng, profile = 'driving') {
  const isLoaded = await ensureGoogleMapsLoaded();
  if (!isLoaded) return null;
  return new Promise(resolve => {
    ...
    directionsService.route({
      ...
      const routes = response.routes.slice(0, 3).map((route, index) => mapGoogleDirectionsRouteCandidate(route, fromLat, fromLng, index)).filter(route => Array.isArray(route.path) && route.path.length > 1);
      if (!routes.length) {
        return resolve(null);
      }
      resolve({
...
export let currentCountryCode = 'IN'; // ── Performance Cache ─────────────────────────────────────────
export const heatmapCache = new Map();
export const CACHE_DURATION = 300000; // 5 minutes
// ── Request Throttling ────────────────────────────────────────
export const REQUEST_DEFAULT_DELAY_MS = 50;
export const REQUEST_DELAY_BY_HOST_MS = {
  'nominatim.openstreetmap.org': 200
};
export const requestHostTimestamps = new Map();
export const RISK_MODEL_STORAGE_KEY = 'safezoneRiskModel';
export const FETCH_TIMEOUT_MS = 5000;
export const OVERPASS_ENDPOINTS = ['https://overpass-api.de/api/interpreter', 'https://overpass.kumi.systems/api/interpreter', 'https://lz4.overpass-api.de/api/interpreter'];
export const GOOGLE_PLACES_ENDPOINT = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json';
export const GOOGLE_PLACES_MAX_RADIUS = 50000;
export const GOOGLE_PLACES_CALLBACK_TIMEOUT_MS = 6500; // India-specific crime categories (IPC sections mapping)
export const THEFT_CATEGORIES = new Set(['theft', 'burglary', 'robbery', 'dacoity', 'vehicle-theft', 'shoplifting', 'pickpocketing', 'auto-theft', 'two-wheeler-theft']);
export const VIOLENT_CATEGORIES = new Set(['murder', 'attempt-to-murder', 'molestation', 'rape', 'assault', 'rioting', 'communal-violence', 'public-order', 'possession-of-weapons', 'criminal-intimidation']); // India state coordinates for regional filtering
export const INDIA_STATE_BOUNDS = {
  'karnataka': {
    latMin: 11.5,
    latMax: 18.5,
    lngMin: 74.0,
...
export const RISK_SIGNAL_MAX_RADIUS_METERS = 2600;
export const HOTSPOT_DEDUPLICATION_GRID_SIZE = 0.00045;
export function getThrottleHostKey(url) {
  const raw = typeof url === 'string' ? url : url && typeof url.url === 'string' ? url.url : '';
  if (!raw) return 'default';
  try {
    const base = typeof window !== 'undefined' && window.location && window.location.href ? window.location.href : 'https://safezone.local';
...
export function getRequestDelayMsForHost(hostKey) {
...
export async function fetchOverpassJson(query, timeoutMs = FETCH_TIMEOUT_MS) {
...
export function normalizeGooglePlacesRadius(radius, fallbackRadius = 3000) {
...
export async function fetchGoogleNearbyPlaces(lat, lng, radius, options = {}) {
  const isLoaded = await ensureGoogleMapsLoaded();
  if (!isLoaded) return [];
  return new Promise(resolve => {
...
export function mapGoogleAmenityResults(results, amenityType, userLat, userLng) {
  const mapped = [];
...
export function dedupeGooglePlacesResults(results) {
  const deduped = [];
  const seen = new Set();
  (results || []).forEach(place => {
    ...
    const key = String(place && place.place_id ? place.place_id : `${place && place.name ? place.name : ''}:${lat}:${lng}`);
...
export function mapGoogleCameraResults(results, userLat, userLng) {
  const cameras = [];
  (results || []).forEach((place, i) => {
    const location = place && place.geometry ? place.geometry.location : null;
    const lat = location ? typeof location.lat === 'function' ? Number(location.lat()) : Number(location.lat) : NaN;
    const lng = location ? typeof location.lng === 'function' ? Number(location.lng()) : Number(location.lng) : NaN;
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) return;
    const businessStatus = String(place.business_status || '').toUpperCase();
    const status = businessStatus === 'CLOSED_TEMPORARILY' || businessStatus === 'CLOSED_PERMANENTLY' ? 'maintenance' : 'active';
    cameras.push({
...
export async function fetchNearbyAmenitiesWithGoogle(lat, lng, radius = 3000) {
  const services = {
    police: [],
    hospital: [],
    fire: []
  ...
  lookupResults.forEach(({
    ...
    if (lookup.amenityType === 'police') services.police = mapped;else if (lookup.amenityType === 'hospital') services.hospital = mapped;else services.fire = mapped;
  });
  return services;
}
export async function fetchNearbyCamerasWithGoogle(lat, lng, radius = 2000) {
  const keywords = ['cctv', 'surveillance'];
  const rawResults = [];
  const keywordResults = await Promise.all(keywords.map(keyword => fetchGoogleNearbyPlaces(lat, lng, radius, {
    keyword,
...
export async function fetchNearbyAmenities(lat, lng, radius = 3000) {
  if (hasGoogleApiKey()) {
    try {
      const googleServices = await fetchNearbyAmenitiesWithGoogle(lat, lng, radius);
      const totalGoogleResults = googleServices.police.length + googleServices.hospital.length + googleServices.fire.length;
  ...
  try {
    ...
    return {
      ...generateFallbackServices(lat, lng),
      error: 'API_FAILED'
    };
  }
}
export function parseOverpassResults(elements, userLat, userLng) {
  const services = {
    police: [],
    hospital: [],
    fire: []
  ...
  elements.forEach((el, i) => {
    ...
    if (amenity === 'police') services.police.push(entry);else if (amenity === 'hospital') services.hospital.push(entry);else if (amenity === 'fire_station') services.fire.push(entry);
  });
...
export function getEmergencyNumber(amenity) {
...
export function generateFallbackServices(lat, lng) {
  const nums = EMERGENCY_NUMBERS[state.currentCountryCode] || EMERGENCY_NUMBERS.DEFAULT;
  const services = {
    police: [],
    hospital: [],
...
export function generateFallbackCameras(lat, lng) {
  const cameras = [];
...
export async function reverseGeocode(lat, lng) {
  ...
  try {
    const response = await fetchWithTimeout(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=16&addressdetails=1`, {
      headers: {
        'Accept-Language': 'en'
      }
    }, 8000);
    ...
    if (data.address && data.address.country_code) {
      state.currentCountryCode = data.address.country_code.toUpperCase();
    }
    return {
      name: data.address ? data.address.neighbourhood || data.address.suburb || data.address.city_district || data.address.town || data.address.city || data.display_name.split(',')[0] : 'Unknown Area',
      fullAddress: data.display_name || `${lat.toFixed(5)}, ${lng.toFixed(5)}`,
      area: data.address ? data.address.city || data.address.town || data.address.state || '' : '',
...
export function pseudoRandomFromCoords(lat, lng, salt = 0) {
  const n = Math.sin(lat * 12.9898 + lng * 78.233 + salt) * 43758.5453;
  return n - Math.floor(n);
}
export function extractElementCoordinates(element) {
...
export function getDistanceDecayWeight(distanceMeters, nearRadius = 180, farRadius = RISK_SIGNAL_MAX_RADIUS_METERS) {
...
export function normalizeAndRankHotspots(hotspots, centerLat, centerLng, limit = 40) {
...
export function getCrimeRecencyWeight(monthText) {
...
export function getCrimeSignalReliability(crimeData) {
...
export function getAccidentSignalReliability(accidentData) {
...
export function buildRegionalCrimeProxySignals(elements, lat, lng) {
  ...
  (elements || []).forEach(el => {
    ...
    if (highway === 'bus_stop' || railway === 'station' || railway === 'halt' || railway === 'subway_entrance' || transitAmenities.has(amenity)) {
      transit += 1;
...
export function generateFallbackCrimeProxySignals(lat, lng) {
...
export async function fetchRegionalCrimeProxySignals(lat, lng, radius = 2200) {
  const query = `
    [out:json][timeout:10];
    (
  ...
  try {
    const {
      data
    } = await fetchOverpassJson(query, 10000);
...
export function saveRiskModel(model) {
  if (typeof localStorage === 'undefined') {
    return;
  }
  try {
...
export function trainRiskModel(crimeData, accidentData, reliability = 0.5) {
...
export function generateIndiaCrimeSynthetic(lat, lng, isKarnataka = false) {
  ...
  for (let i = 0; i < crimeCount; i++) {
    ...
    const crimeTypes = isKarnataka ? ['theft', 'molestation', 'robbery', 'vehicle-theft', 'assault'] : ['theft', 'robbery', 'molestation', 'dacoity', 'rioting', 'assault', 'vehicle-theft'];
    ...
    crimes.push({
      ...
      category: category,
      month: crimeDate.toISOString().slice(0, 7),
      // YYYY-MM format
...
export function getCrimeIPCSection(category) {
...
export function processCrimeData(crimes, lat, lng, source) {
...
export async function fetchAccidentRiskSignals(lat, lng, radius = 2000) {
  ...
  try {
    const {
      data
    } = await fetchOverpassJson(query, 7000);
    const hotspots = [];
    let hazardCount = 0;
    let signalCount = 0;
    let hazardScore = 0;
    ...
    (data.elements || []).forEach(el => {
      ...
      if (hazardTag) {
        hazardCount += 1;
        let severity = 1.8;
        if (hazardTag.includes('dangerous_curve')) severity = 2.9;else if (hazardTag.includes('slippery')) severity = 2.3;else if (hazardTag.includes('falling_rocks')) severity = 3.1;else if (hazardTag.includes('accident')) severity = 2.4;
      ...
      signalCount += 1;
      const conflictSeverity = isSignal ? 0.58 : highwayTag === 'crossing' ? 0.75 : 0.88;
      conflictScore += conflictSeverity * distanceWeight;
      const label = isSignal ? 'traffic signals' : highwayTag === 'crossing' ? 'crossing conflict point' : 'junction conflict point';
      hotspots.push({
        lat: coords.lat,
        lng: coords.lng,
        title: label,
...
export async function fetchPublicSafetyRisk(lat, lng, radius = 2500) {
  try {
    ...
    const mergedHotspots = normalizeAndRankHotspots([...(crimeData.hotspots || []), ...(accidentData.hotspots || [])], lat, lng, 50);
    return {
...
export function mergeRiskDataWithBackendAssessment(riskData = {}, backendResponse = null) {
  if (!backendResponse || typeof backendResponse !== 'object') {
    return riskData;
  }
  const baseRisk = riskData && typeof riskData === 'object' ? riskData : {};
  const scoring = backendResponse.scoring && typeof backendResponse.scoring === 'object' ? backendResponse.scoring : {};
  const cv = backendResponse.cv && typeof backendResponse.cv === 'object' ? backendResponse.cv : {};
  const sceneRisk = cv.sceneRisk && typeof cv.sceneRisk === 'object' ? cv.sceneRisk : {};
  const deploymentReadiness = scoring.deploymentReadiness && typeof scoring.deploymentReadiness === 'object' ? scoring.deploymentReadiness : null;
  ...
  const cvQuality = Number.isFinite(cvConfidenceRaw) ? Number((clampRiskValue(cvConfidenceRaw, 0, 1) * 100).toFixed(1)) : null;
  const backendFactors = Array.isArray(scoring.factors) ? scoring.factors.map(value => String(value || '').trim()).filter(Boolean).slice(0, 8) : [];
  ...
  const recommendations = Array.isArray(backendResponse.recommendations) ? backendResponse.recommendations.slice(0, 6).map(item => String(item || '').trim()).filter(Boolean) : Array.isArray(scoring.recommendations) ? scoring.recommendations.slice(0, 6).map(item => String(item || '').trim()).filter(Boolean) : [];
  ...
  const merged = {
    ...
    reliabilityScore: Number.isFinite(confidenceScore) ? Number(confidenceScore.toFixed(1)) : Number(baseRisk.reliabilityScore || 0),
    ...
    cvSignals: {
      ...
      score: Number.isFinite(Number(sceneRisk.score)) ? Math.round(clampRiskValue(Number(sceneRisk.score), 0, 100)) : 0,
      confidence: Number.isFinite(cvConfidenceRaw) ? Number((clampRiskValue(cvConfidenceRaw, 0, 1) * 100).toFixed(1)) : 0,
      detections: Array.isArray(cv.detections) ? cv.detections.length : 0,
      objectCounts: sceneRisk.objectCounts && typeof sceneRisk.objectCounts === 'object' ? sceneRisk.objectCounts : {},
      signals: Array.isArray(sceneRisk.signals) ? sceneRisk.signals.slice(0, 4).map(item => String(item || '').trim()).filter(Boolean) : []
    },
    recommendations,
    sources: {
      ...existingSources,
...
export const ROUTE_OPTIMIZATION_MODES = new Set(['balanced', 'fastest', 'safest', 'least-congested']);
export function normalizeRouteOptimizationMode(mode) {
  const value = String(mode || 'balanced').trim().toLowerCase();
  return ROUTE_OPTIMIZATION_MODES.has(value) ? value : 'balanced';
}
export function clampRouteMetric(value, min, max) {
  if (!Number.isFinite(value)) return min;
  return Math.max(min, Math.min(max, value));
}
export function sampleRoutePoints(path, maxPoints = 18) {
...
export function estimateRouteHotspotExposure(path, riskData = null) {
...
export function getCongestionTimeProfile(hour) {
...
export function predictRouteCongestion(routeCandidate, profile = 'driving', options = {}) {
  ...
  const {
    nearHotspots,
    hotspotExposure
  } = estimateRouteHotspotExposure(routeCandidate.path, riskData);
  ...
  const seed = pseudoRandomFromCoords(Number(firstPoint[0] || 0) + Number(lastPoint[0] || 0), Number(firstPoint[1] || 0) + Number(lastPoint[1] || 0), Number(routeCandidate.distance || 0) / 1000 + Number(routeCandidate.duration || 0) / 60);
  const deterministicNoise = (seed - 0.5) * 8;
  const riskPenalty = riskData ? clampRouteMetric(Number(riskData.accidentHotspots || 0) * 1.1 + Number(riskData.conflictPoints || 0) * 0.7 + Number(riskData.penalty || 0) * 0.45, 0, 22) : 0;
  ...
  let level = 'moderate';
  if (congestionScore < 35) level = 'low';else if (congestionScore < 60) level = 'moderate';else if (congestionScore < 80) level = 'high';else level = 'severe';
...
export function mapGoogleDirectionsRouteCandidate(route, fallbackLat, fallbackLng, index = 0) {
  const leg = route && Array.isArray(route.legs) && route.legs.length > 0 ? route.legs[0] : null;
  const endLat = leg && leg.end_location ? Number(typeof leg.end_location.lat === 'function' ? leg.end_location.lat() : leg.end_location.lat) : Number(fallbackLat);
  const endLng = leg && leg.end_location ? Number(typeof leg.end_location.lng === 'function' ? leg.end_location.lng() : leg.end_location.lng) : Number(fallbackLng);
  const path = Array.isArray(route && route.overview_path) ? route.overview_path.map(point => [Number(typeof point.lat === 'function' ? point.lat() : point.lat), Number(typeof point.lng === 'function' ? point.lng() : point.lng)]).filter(point => Number.isFinite(point[0]) && Number.isFinite(point[1])) : [];
  const steps = (leg && Array.isArray(leg.steps) ? leg.steps : []).map((step, i) => {
    ...
    const stepLat = step.end_location ? Number(typeof step.end_location.lat === 'function' ? step.end_location.lat() : step.end_location.lat) : endLat;
    const stepLng = step.end_location ? Number(typeof step.end_location.lng === 'function' ? step.end_location.lng() : step.end_location.lng) : endLng;
    return {
      index: i,
      instruction,
...
export function mapOsrmRouteCandidate(route, fallbackLat, fallbackLng, index = 0) {
  const leg = route && Array.isArray(route.legs) && route.legs[0] ? route.legs[0] : {
    steps: []
  };
  const path = route && route.geometry && Array.isArray(route.geometry.coordinates) ? route.geometry.coordinates.map(pair => [Number(pair[1]), Number(pair[0])]).filter(point => Number.isFinite(point[0]) && Number.isFinite(point[1])) : [];
  ...
  const steps = (Array.isArray(leg.steps) ? leg.steps : []).map((step, i) => {
    const maneuverLoc = step.maneuver && Array.isArray(step.maneuver.location) ? step.maneuver.location : [Number(fallbackLng), Number(fallbackLat)];
...
export function getRouteOptimizationWeights(mode) {
  if (mode === 'fastest') {
    return {
      eta: 0.62,
      distance: 0.20,
...
export function calculateRouteOptimizationScore(candidate, stats, mode, options = {}) {
  ...
  const congestionSafetyPenalty = congestionLevel === 'severe' ? 30 : congestionLevel === 'high' ? 18 : congestionLevel === 'moderate' ? 8 : 2;
  let safetyPenaltyRaw = hotspotExposure * 82 + nearHotspots * 4.6 + riskPenaltyBase * (mode === 'safest' ? 1.25 : 0.8) + congestionSafetyPenalty;
  if (edgeAiScore > 0) {
  ...
  const optimizationScore = etaNorm * weights.eta + distanceNorm * weights.distance + congestionNorm * weights.congestion + safetyNorm * weights.safety;
  return {
...
export function optimizeRouteAlternatives(alternatives, mode = 'balanced', profile = 'driving', options = {}) {
...
export function buildRouteBundle(source, alternatives, mode, profile, options = {}) {
  ...
  const selectedRoute = selectedIndex >= 0 ? optimized.alternatives[selectedIndex] : optimized.alternatives[0];
  return {
    source,
...
export function formatRouteInstruction(step) {
...
export async function fetchRouteDirections(fromLat, fromLng, toLat, toLng, profile = 'driving', options = {}) {
  ...
  if (hasGoogleApiKey()) {
    try {
      ...
      if (googleRoute && Array.isArray(googleRoute.alternatives) && googleRoute.alternatives.length > 0) {
        const optimizedGoogle = buildRouteBundle(googleRoute.source || 'google-directions-sdk', googleRoute.alternatives, optimizationMode, profile, options);
        if (optimizedGoogle) {
  ...
  try {
    ...
    const alternatives = data.routes.slice(0, 3).map((route, index) => mapOsrmRouteCandidate(route, fromLat, fromLng, index)).filter(route => Array.isArray(route.path) && route.path.length > 1);
...
export function getHeatmapData(hour, center = MAP_CENTER, span = 0.05) {
  ...
  for (let i = 0; i < numHotspots; i++) {
    ...
    for (let j = 0; j < 6; j++) {
      points.push([lat + (Math.random() - 0.5) * 0.008, lng + (Math.random() - 0.5) * 0.008, intensity * (0.3 + Math.random() * 0.4)]);
    }
  }

  // Ambient risk
  for (let i = 0; i < 25; i++) {
    points.push([center[0] + (Math.random() - 0.5) * span * 2.5, center[1] + (Math.random() - 0.5) * span * 2.5, (0.03 + Math.random() * 0.12) * nightMultiplier]);
  }
...
export async function fetchNearbyProperties(lat, lng, radius = 1200) {
  ...
  for (let i = 0; i < numProps; i++) {
    ...
    properties.push({
      ...
      lat: lat + (Math.random() - 0.5) * (radius / 55000),
      // Approximate offset
      ...
      image: `https://images.unsplash.com/photo-${['1512917774080-9991f1c4c750', '1600596542815-ffad4c1539a9', '1564013799919-ab600027ffc6', '1522708323590-d24dbb6b0267', '1580587771525-78b9dba3b914', '1449844908441-8829872d2607'][Math.floor(Math.random() * 6)]}?auto=format&fit=crop&w=300&q=80`
    });
  }
